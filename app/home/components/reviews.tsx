    ...
    >
      <div className="flex flex-row items-center gap-2">
        ...
        <img
          ...
          height="32"
          alt=""
          src={`https://ui-avatars.com/api/?name=${name[0]}+${name[1][0]}&background=F3F1EB`}
        />
...
export function Reviews() {
  return (
    <div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-lg border border-stone-300/[0.7] py-8 sm:shadow-sm">
      <Subheading className="mb-4 inline-flex items-center">
        {reviews.title}
      </Subheading>

      <Marquee pauseOnHover className="select-none [--duration:50s]">
        {firstRow.map((review) => (
          <ReviewCard key={review.name} {...review} />
        ))}
      </Marquee>

      <Marquee reverse pauseOnHover className="select-none [--duration:55s]">
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 w-1/6 bg-gradient-to-r from-[#F3F1EB]"></div>
      <div className="pointer-events-none absolute inset-y-0 right-0 w-1/6 bg-gradient-to-l from-[#F3F1EB]"></div>
    </div>
  )
}
